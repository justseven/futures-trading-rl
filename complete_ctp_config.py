...
def complete_ctp_connection(config: Dict[str, Any]):
    """使用完整键名的CTP连接"""
    print(f"\n正在连接CTP服务器...")
    print(f"交易服务器: {config['交易服务器']}")
    print(f"行情服务器: {config['行情服务器']}")
    print(f"用户ID: {config['用户名']}")
    print(f"经纪商: {config['经纪商代码']}")
    print(f"柜台环境: {config['柜台环境']}")
    
    # 分别测试交易和行情服务器地址
    td_parts = config['交易服务器'].replace('tcp://', '').split(':')
    md_parts = config['行情服务器'].replace('tcp://', '').split(':')
...
def main():
    ...
    # 使用完整的正确中文键名，根据错误日志确定
    config = {
        "用户名": userid,
        "密码": password,
        "经纪商代码": "9999",
        "交易服务器": "tcp://182.254.243.31:40011",  # SimNow模拟交易地址
        "行情服务器": "tcp://182.254.243.31:40001",  # SimNow行情地址
        "产品名称": "CTP_SIMNOW",
        "授权编码": "0000000000000000",  # SimNow认证码
        "应用ID": "simnow_client_test",
        "柜台环境": "实盘"  # 对于SimNow，应该是模拟环境
    }
    
    # 执行连接
    result = complete_ctp_connection(config)
